...
export default function Alphabets() {
  ...
  useEffect(() => {
    console.log('Fetching quizzes for Alphabets...'); // Debugging log
    fetch('https://trainingwebsite-apot.onrender.com/api/get-quizzes-with-answer?category=Alphabets')
      .then((response) => {
      ...
      .then((data) => {
        ...
        const filteredQuizzes = data.filter((quiz) => quiz.type === 'Alphabets');
        setQuizzes(filteredQuizzes);
        setIsLoading(false);
      })
      .catch((error) => {
        console.error('Error fetching quizzes:', error); // Debugging log
        setIsLoading(false);
      });
  }, []);
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="bg-white shadow-md rounded-lg p-6 w-full max-w-2xl">
        <h1 className="text-2xl font-bold mb-6 text-center">Alphabets</h1>
        {quizzes.map((quiz) => (
