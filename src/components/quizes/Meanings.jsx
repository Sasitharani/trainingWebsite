...
export default function Meanings() {
  ...
  useEffect(() => {
    fetch('https://trainingwebsite-apot.onrender.com/api/get-quizzes-with-answer?category=Meanings')
      .then((response) => response.json())
      .then((data) => {
        const filteredQuizzes = data.filter((quiz) => quiz.type === 'Meanings');
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-gray-100">
      <div className="bg-white shadow-md rounded-lg p-6 w-full max-w-2xl">
        <h1 className="text-2xl font-bold mb-6 text-center">Meanings</h1>
        {quizzes.map((quiz) => (
